...
class Food:
    def __init__(self, is_big=False, snake_body=None):
        self.is_big = is_big
        self.lifetime = 300 if is_big else -1
        self.position = self.generate_position(snake_body)

    def generate_position(self, snake_body=None):
        while True:
            x = random.randint(0, (WIDTH - BLOCK_SIZE * (2 if self.is_big else 1)) // BLOCK_SIZE) * BLOCK_SIZE
            y = random.randint(0, (HEIGHT - BLOCK_SIZE * (2 if self.is_big else 1)) // BLOCK_SIZE) * BLOCK_SIZE
            if snake_body:
                blocks_to_check = [(x + dx * BLOCK_SIZE, y + dy * BLOCK_SIZE) for dx in range(2 if self.is_big else 1) for dy in range(2 if self.is_big else 1)]
                if any(block in snake_body for block in blocks_to_check):
                    continue
            return (x, y)
...
class Game:
    def __init__(self):
        ...
        self.small_food_eaten = 0  # Küçük yem sayacı

    def reset(self):
        self.snake = Snake()
        self.foods = [Food(snake_body=self.snake.body)]
        self.paused = False
        self.small_food_eaten = 0
        ...
        for i, food in enumerate(self.foods[:]):
            ...
            if head_rect.colliderect(food_rect):
                if food.is_big:
                    ...
                    if sounds["big_eat"]:
                        sounds["big_eat"].play()
                    self.small_food_eaten = 0  # Büyük yem yendiğinde sayaç sıfırlanır
                else:
                    self.snake.score += 1
                    self.snake.grow(1)
                    if sounds["eat"]:
                        sounds["eat"].play()
                    self.small_food_eaten += 1  # Küçük yem sayacı artırılır

                self.foods.pop(i)
                self.foods.append(Food(snake_body=self.snake.body))
                ...
                # 5 küçük yem yendi ve ekranda büyük yem yoksa, büyük yem ekle
                if self.small_food_eaten >= 5 and not any(f.is_big for f in self.foods):
                    self.foods.append(Food(is_big=True, snake_body=self.snake.body))
                    self.small_food_eaten = 0
    ...
    def draw_game_over(self):
        ...
        highscore_text = font_medium.render(f"Rekor: {self.high_score}", True, WHITE)
        restart_text = font_small.render("1. Tekrar Oyna", True, WHITE)
        menu_text = font_small.render("2. Ana Menü", True, WHITE)
        quit_text = font_small.render("3. Çıkış", True, WHITE)

        screen.blit(gameover_text, (WIDTH // 2 - gameover_text.get_width() // 2, 100))
        screen.blit(score_text, (WIDTH // 2 - score_text.get_width() // 2, 180))
        screen.blit(highscore_text, (WIDTH // 2 - highscore_text.get_width() // 2, 230))
        screen.blit(restart_text, (WIDTH // 2 - restart_text.get_width() // 2, 300))
        screen.blit(menu_text, (WIDTH // 2 - menu_text.get_width() // 2, 340))
